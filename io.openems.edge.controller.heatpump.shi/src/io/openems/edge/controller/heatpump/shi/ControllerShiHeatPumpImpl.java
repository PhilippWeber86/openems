...
import io.openems.edge.controller.heatpump.shi.BatteryReserveCalculator.Result;
...
public class ControllerShiHeatPumpImpl extends AbstractOpenemsComponent
	...
	private int calculateSpareEssEnergy() {
		var result = this.calculateBatteryReserve();
		this._setNoPredictionAvailable(result.reason().isForecastProblem());
		// The reserve is null unless a complete forecast produced one, so a failed
		// cycle never leaves a stale value from a previous successful one behind.
		this._setNightReserveEnergy(result.reserveEnergy());
		return result.spareEnergy();
	}

	/**
	 * Fetches the forecasts and hands them to the {@link BatteryReserveCalculator}.
	 * The order of the guards matters: a battery with nothing to give is not a
	 * forecast problem and must not raise the forecast warning, so it is settled
	...
	private BatteryReserveCalculator.Result calculateBatteryReserve() {
		...
		var calculator = new BatteryReserveCalculator(this.config.nightReserveMode(), this.config.minSoc(),
				this.config.nightReserveBuffer(), this.forecastChargePower(),
		...
		if (this.predictorManager == null) {
			return Result.of(Reason.FORECAST_UNAVAILABLE);
		}
		var productionPrediction = this.getProductionPrediction();
		...
		// The same quarter rounding as the prediction keys lines the lookups up
		// exactly; the calculator then aligns everything by time, not by index.
		return calculator.calculate(usableEnergy, essCapacity,
				DateUtils.roundDownToQuarter(Instant.now(this.componentManager.getClock())),
				this.remainingHoursOfCurrentQuarter(), productionPrediction.toMapWithAllQuarters(),
	 ...
	 *
	 * <p>
	 * Taken from {@code _sum/EssMinDischargePower}: the maximum charge power ever
	 * MEASURED, as a negative value. It needs no assumption about the battery -
	 * Core.Sum maintains it as a running minimum of {@code EssDischargePower}, so a
	 * full battery reporting 0 W cannot lower it, and persists it through the
	 * Core.Sum configuration, so it survives a restart. The Energy Scheduler's
	 * {@code GocBuilder} derives its own maximum charge power from the same Channel.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Until a plant has ever charged the value is 0, and then NO future recharge is
	 * credited. That is deliberate rather than a gap to be filled with an estimate:
	 * the honest answer while no evidence exists is the conservative one, the same
	 * rule this Controller follows for a missing forecast or a missing measurement.
	 * It resolves itself within hours of the first charge, and the absolute override
	 * is there for anyone who wants to bridge it explicitly.
	 *
	 * <p>
	 * One caveat: the measured value is the maximum EVER and never decays, so an
	 * ageing battery keeps a historical peak. That over-credits slightly, which is
	 * the unsafe direction - the override is the way out.
	 *
	...
	private int forecastChargePower() {
		...
		return Math.abs(this.sum.getEssMinDischargePower().orElse(0));
	}

	/**
	 * The still-unelapsed fraction of the current quarter in hours, so the first
	 * forecast quarter is only counted for the part of it that is still ahead.
	 *
	 * @return the remaining fraction of the current quarter in hours
	 */
	private float remainingHoursOfCurrentQuarter() {
		var now = ZonedDateTime.now(this.componentManager.getClock());
		return Math.max(0F, (15F - now.get(ChronoField.MINUTE_OF_HOUR) % 15F - now.getSecond() / 60F) / 60F);
	}
