...
@interface Config {
	...
	@AttributeDefinition(name = "Maximum forecast charge power", description = "Optional absolute override in W for the battery charge power credited to the night-reserve forecast. Normally leave at 0: the value is taken from _sum/EssMinDischargePower, the maximum charge power ever MEASURED, which Core.Sum keeps as a running minimum (a full battery reporting 0 W cannot lower it) and persists across restarts. Set this when that measured maximum has become optimistic - it is the maximum EVER and never decays, so an ageing battery keeps a historical peak - or to bridge the first hours of a plant that has not charged yet, where the measured value is still 0 and no future recharge is credited at all.")
