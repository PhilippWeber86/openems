...
@interface Config {
	...
	@AttributeDefinition(name = "Forecast charge rate [C]", description = "Battery charge rate the night-reserve forecast assumes the plant can sustain, as a multiple of its capacity per hour (0.5 C on a 10 kWh battery = 5000 W). Bounds how much of a predicted PV surplus may be credited as recharge: without it, an hour of 10 kW predicted surplus counts as 10 kWh even on a battery that only absorbs 1 kW, and the reserve comes out too small. Derived from the capacity on purpose, because that is a standard ESS Channel and the reserve calculation already requires it - so this stays independent of the inverter make, unlike the momentary AllowedChargePower (0 W on a full battery) or MaxApparentPower (the inverter rating, not the battery's). 0.5 C is a typical continuous rate for home storage; lower it if the battery is slower, which only makes the reserve larger and is therefore the safe direction.")
	double forecastChargeCRate() default 0.5;

	@AttributeDefinition(name = "Maximum forecast charge power", description = "Optional absolute override in W for the charge power credited to the forecast, for a plant whose charge power is known and does not follow from its capacity. 0 = derive it from the capacity and the charge rate above.")
