...
@interface Config {
	...
	@AttributeDefinition(name = "Forecast charge rate [C]", description = "Fallback charge rate for a plant that has never charged yet, as a multiple of the battery capacity per hour (0.5 C on a 10 kWh battery = 5000 W). Normally unused: the charge power credited to the night-reserve forecast is taken from _sum/EssMinDischargePower, the maximum charge power ever MEASURED, which Core.Sum keeps as a running minimum (a full battery reporting 0 W cannot lower it) and persists across restarts. This rate only bridges the time until that value exists. Bounding the credited recharge matters because otherwise an hour of 10 kW predicted surplus counts as 10 kWh even on a battery that only absorbs 1 kW, and the reserve comes out too small.")
	double forecastChargeCRate() default 0.5;

	@AttributeDefinition(name = "Maximum forecast charge power", description = "Optional absolute override in W for the charge power credited to the forecast. Overrides both the measured maximum and the charge rate. Useful when the measured maximum has become optimistic - it is the maximum EVER and never decays, so an ageing battery keeps a historical peak. 0 = use the measured value, falling back to the charge rate.")
