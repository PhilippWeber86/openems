...
class ControllerShiHeatPumpImplTest {
	...
	/**
	 * One physical situation, described the two ways the ESS can report it, with the
	 * heat pump on the grid side.
	 *
	 * <p>
	 * PV 2000 W, battery idle, household 800 W, so the grid meter reads -1200 W. The
	 * heat pump draws 2500 W upstream of that meter, i.e. 1300 W of it are uncovered
	 * and have to be forced out of the battery.
	 *
	 * <p>
	 * AC-coupled the ESS reports the battery on EssActivePower, so 0 W. As a
	 * HybridEss it reports the whole inverter, so 2000 W. Almost every intermediate
	 * figure therefore differs - the forced-export TARGET is -1200+1300 = 100 W in
	 * one case and 800+1300 = 2100 W in the other - yet the battery power actually
	 * demanded is the same 1300 W, because the target is expressed in whatever
	 * Active-Power counts and the controller subtracts the very same baseline again.
	 *
	 * <p>
	 * That invariance is the reason the grid-side path needed no change for the
	 * hybrid rework. It was argued on paper before; this pins it.
	 *
	 * @throws Exception on error
	 */
	@Test
	void testGridSideForcedExportIsInvariantToTheEssModel() throws Exception {
		for (var essActivePower : new int[] { 0 /* AC-coupled */, 2000 /* HybridEss */ }) {
			var clock = createDummyClock();
			var cm = new DummyComponentManager(clock);
			var sum = new DummySum();
			new ControllerTest(new ControllerShiHeatPumpImpl()) //
					...
					.activate(MyConfig.create() //
							...
							.setMinimumSurplusPowerForElevatedMode(4000) //
							.build()) //
					.next(new TestCase("EssActivePower " + essActivePower + " W") //
							.input("_sum", Sum.ChannelId.GRID_ACTIVE_POWER, -1200) //
							// the battery is idle in BOTH readings - that is the figure the
							// surplus is built on, and it does not change with the model
							.input("_sum", Sum.ChannelId.ESS_DISCHARGE_POWER, 0) //
							.input("_sum", Sum.ChannelId.ESS_ACTIVE_POWER, essActivePower) //
							.input("_sum", Sum.ChannelId.ESS_SOC, 65) //
							.input("_sum", Sum.ChannelId.ESS_CAPACITY, 10_000) //
							.input("heatPump0", ElectricityMeter.ChannelId.ACTIVE_POWER, 2500) //
							.input("heatPump0", HeatShiHeatPump.ChannelId.OPERATING_MODE_STATUS, 1) //
							...
							.input("heatPump0", HeatShiHeatPump.ChannelId.HOT_WATER_MODE, 0) //
							.input("heatPump0", HeatShiHeatPump.ChannelId.HOT_WATER_ACTIVE_SETPOINT, 480) //
							.output(ControllerShiHeatPump.ChannelId.ESS_FORCED_EXPORT_POWER, 1300)) //
					.deactivate();
		}
	}
	...
	/**
	 * Runs one cycle of the {@link #rechargeWindowPredictor} forecast with the given
	 * measured charge-power maximum, and asserts the resulting free energy.
	 *
	 * @param measuredChargePower the maximum charge power ever measured in W
	 *                            (negative); 0 = never charged
	 * @param absoluteOverride    the absolute override in W; 0 = use the measurement
	 * @param expectedFreeEnergy  the expected free battery energy in Wh
	 * @throws Exception on error
	 */
	private static void assertFreeEnergyWithMeasuredChargePower(int measuredChargePower, int absoluteOverride,
		...
		new ControllerTest(new ControllerShiHeatPumpImpl()) //
				...
				.activate(MyConfig.create() //
						...
						.setForecastDischargeEfficiency(100) //
						.build()) //
				.next(new TestCase("measured " + measuredChargePower + " W, override " + absoluteOverride + " W") //
						.input("_sum", Sum.ChannelId.GRID_ACTIVE_POWER, 0) //
						.input("_sum", Sum.ChannelId.ESS_DISCHARGE_POWER, 0) //
						.input("_sum", Sum.ChannelId.ESS_ACTIVE_POWER, 0) //
						.input("_sum", Sum.ChannelId.ESS_SOC, 65) //
	...
	@Test
	void testForecastChargePowerComesFromTheMeasuredMaximum() throws Exception {
		...
		// persists it through its own configuration. No assumption about the battery is
		// needed anywhere. The Energy Scheduler's GocBuilder uses the same Channel.
		//
		// 1 kW measured turns the forecast's 10 kWh window into 1 kWh of credited
		// recharge, so the cumulative deficit is 3000 Wh of the 5000 Wh usable.
		assertFreeEnergyWithMeasuredChargePower(-1000, 0, 2000);
		// A higher measured maximum credits more - the sign is a magnitude here.
		assertFreeEnergyWithMeasuredChargePower(-10_000, 0, 3000);
		// The absolute override outranks the measurement, for the case where the
		// maximum EVER has become optimistic for an ageing battery.
		assertFreeEnergyWithMeasuredChargePower(-10_000, 1000, 2000);
	}

	@Test
	void testNoMeasuredChargePowerCreditsNoRecharge() throws Exception {
		// A plant that has never charged has no evidence of what its battery absorbs,
		// so NOTHING is credited - deliberately, rather than filling the gap with an
		// estimate. It is the same rule the Controller follows for a missing forecast
		// or a missing measurement, and it resolves itself within hours of the first
		// charge. Both deficits then stand: 4000 Wh reserved, 1000 Wh free.
		assertFreeEnergyWithMeasuredChargePower(0, 0, 1000);
		// The override is the way to bridge that first window explicitly.
		assertFreeEnergyWithMeasuredChargePower(0, 1000, 2000);
	}
